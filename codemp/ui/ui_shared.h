...
typedef struct editFieldDef_s {
	...
	char buffer[2048];			// used in case no cvar is set
} editFieldDef_t;
...
#define ITF_TEXTFIELD_PASSWORD 0x0100
#define ITF_HANDLE_ESCAPE 0x0200
#define ITF_TEXTFIELD_ALLOWNEGATIVE 0x0400
#define ITF_TEXTFIELD_ALLOWPERIOD 0x0800
...
typedef struct itemDef_s {
	...
	const char	*onReturn;					// text fields only: Script to run when enter is pressed
	const char	*onEscape;					// text fields only: Script to run when escape is pressed
//JLFACCEPT MPMOVED
	const char  *accept;
//JLFDPADSCRIPT
...
//JLFACCEPT
	...
	// Jedi Knight Galaxies
	int			handleEsc;					// 1 if the menu handles the escape key, if 0, escape closes the window
} menuDef_t;
...
typedef struct uiCrossoverExports_s
{
	...
	void			(*BuybackCheckResult)(qboolean found);
