...
// Jedi Knight Galaxes - Find a friendly player?
qboolean TeamFriendly( int player )
{
	...
	{
		if ( cgs.party.active && cgs.party.members[i].id == player && cgs.party.members[i].status >= 0 )
		{
			return qtrue;
		}
	}

	// Team edit for Team FFA
	if( cgs.gametype >= GT_TEAM )
	{
		if( cgs.clientinfo[player].team == cgs.clientinfo[cg.predictedPlayerState.clientNum].team )
		{
			return qtrue;
		}
	}
...
#define DOUBLE_MAX_SURF_LIST_SIZE	2048
...
static qboolean CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *skinName, const char *teamName, int clientNum ) {
	...
	qboolean humanoid = qfalse;
	qboolean jkgHumanoid = qfalse;
...
retryModel:
	...
	{ //We won't force colors for siege.
		...
		BG_ValidateSkinForTeam( ci->modelName, ci->skinName, ci->team, ci->colorOverride, cgs.redTeam, cgs.blueTeam, clientNumFixed );
	...
	if (!BGPAFtextLoaded)
	{
		...
		if (Q_stricmp(afilename, "models/players/_humanoid/animation.cfg") &&
		    Q_stricmp (afilename, "models/players/_humanoidJKG/animation.cfg") /*&&
			Q_stricmp(afilename, "models/players/rockettrooper/animation.cfg")*/)
		{
			Com_Printf( "Model does not use supported animation config.\n");
			return qfalse;
		}
		else if (humanoid && BG_ParseAnimationFile("models/players/_humanoid/animation.cfg", bgHumanoidAnimations, qtrue) == -1)
		{
			Com_Printf( "Failed to load animation file models/players/_humanoid/animation.cfg\n" );
			return qfalse;
		}
		else if (jkgHumanoid && BG_ParseAnimationFile("models/players/_humanoidJKG/animation.cfg", NULL, qfalse) == -1)
		{
			Com_Printf( "Failed to load animation file models/players/_humanoidJKG/animation.cfg\n" );
			return qfalse;
		}
...
// G2 Util functions
//
// Should really be moved to bg_g2_utils?
//=========================================================
...
void JKG_G2_ShowOnlySelectedSurfaces(void* g2, const char* modelPath, std::vector<std::string> visibleSurfaces) {
	char surfaceName[MAX_QPATH];
	int numSurfaces = JKG_G2_GetNumberOfSurfaces(modelPath);

	// Turn off *ALL* of the surfaces
	...
	// Now turn on only the ones that we want
	for (auto it = visibleSurfaces.begin(); it != visibleSurfaces.end(); ++it) {
		trap->G2API_SetSurfaceOnOff(g2, it->c_str(), 0);
	}
}

void JKG_G2_ShowOnlySelectedSurfaces(void* g2, int numSurfaces, std::vector<std::string> visibleSurfaces) {
	char surfaceName[MAX_QPATH];

	// Turn off _ALL_ surfaces
	for (int i = 0; i < numSurfaces; i++) {
		trap->G2API_GetSurfaceName(g2, i, 0, surfaceName);
		if (surfaceName[0] == '*') {
			// Don't poke surfaces for bolts
			continue;
		}
		trap->G2API_SetSurfaceOnOff(g2, surfaceName, 0x2);
	}

	// Turn on only the ones we want
	for (auto it = visibleSurfaces.begin(); it != visibleSurfaces.end(); ++it) {
		trap->G2API_SetSurfaceOnOff(g2, it->c_str(), 0);
	}
}

void JKG_G2_ShowOnlySelectedSurfaces(void* g2, std::vector<std::string> visibleSurfaces) {
	JKG_G2_ShowOnlySelectedSurfaces(g2, trap->G2API_GetSurfaceCount(g2), visibleSurfaces);
}

void JKG_G2_ShowAllSurfaces(void* g2, int numSurfaces) {
	char surfaceName[MAX_QPATH];
...
void JKG_G2_ShowSelectedSurfaces(void* g2, const std::vector<std::string>& showSurfaces) {
	for (auto it = showSurfaces.begin(); it != showSurfaces.end(); ++it) {
		trap->G2API_SetSurfaceOnOff(g2, it->c_str(), 0);
	}
}

void JKG_G2_HideSelectedSurfaces(void* g2, const std::vector<std::string>& hideSurfaces) {
	for (auto it = hideSurfaces.begin(); it != hideSurfaces.end(); ++it) {
		trap->G2API_SetSurfaceOnOff(g2, it->c_str(), 0x2);
	}
}
...
void CG_ArmorChanged(centity_t* cent, entityState_t* to, entityState_t* from) {
	for (int i = 0; i < MAX_ARMOR; i++) {
		// Check to see whether each value has been changed
		if (to->armor[i] != from->armor[i]) {
			int armor = to->armor[i];
			int prevArmor = from->armor[i];

			// We always need to clear out the previous ghoul2 instance on this slot and also re-enable the surfaces that were disabled
			if (trap->G2API_HasGhoul2ModelOnIndex(&cent->ghoul2, 4 + i)) {
...
void CG_LoadCISounds(clientInfo_t *ci, qboolean modelloaded, int clientNum)
{
	fileHandle_t f;
	...
	if (isFemale)
	{
		...
		if(clientNum == cg.predictedPlayerState.clientNum || clientNum == -1)
		{
			trap->Cvar_Set("sex", "f");
		}
	}
...
void JKG_SwapToSaber(int saberNum, clientInfo_t *ci, const char *newSaber, int weapon, int variation)
{
	...
	trap->G2API_DuplicateGhoul2Instance( BG_GetWeaponGhoul2( weapon, variation ), &ci->ghoul2Weapons[saberNum] );
...
void CG_NewClientInfo( int clientNum, qboolean entitiesInitialized ) {
	...
	// JKG - custom team
	v = Info_ValueForKey( configstring, "ct" );
	newInfo.customteam = atoi( v );
...
//copy team info out to menu
	...
	if ( cg_forceModel.integer ) {
		...
		if ( cgs.gametype >= GT_TEAM ) {
			// keep skin name
			slash = const_cast<char *>(strchr( v, '/' ));
			if ( slash ) {
	...
	//trap->G2API_CleanGhoul2Models(&oldG2Weapons[0]);
	...
	*ci = newInfo;
	...
	{	// Copy the new ghoul2 model to the centity.
		...
		if (anim)
		{
			...
			// Apply it for all of the entity's armor
			for (int i = 0; i < MAX_ARMOR; i++) {
		...
		if (anim)
		{
			...
			// apply to armor as well
			for (int i = 0; i < MAX_ARMOR; i++) {
				trap->G2API_SetBoneAnim(ci->ghoul2Model, 4 + i, "lower_lumbar", firstFrame, anim->firstFrame + anim->numFrames, flags, animSpeed, cg.time, setFrame, 150);
			}
			...
			cg_entities[clientNum].currentState.weapon == WP_SABER)
		{
			...
			{
				CG_CopyG2WeaponInstance(&cg_entities[clientNum], cg_entities[clientNum].currentState.weapon, cg_entities[clientNum].currentState.weaponVariation, cg_entities[clientNum].ghoul2);
				cg_entities[clientNum].ghoul2weapon = CG_G2WeaponInstance(&cg_entities[clientNum], cg_entities[clientNum].currentState.weapon, cg_entities[clientNum].currentState.weaponVariation);
			}
								...
								centity_t *const cent, footstepType_t footStepType ) 
{
	...
	if ( effectID != -1 )
	{
		trap->FX_PlayEffectID( effectID, trace.endpos, trace.plane.normal, -1, -1, false );
	}
...
void CG_PlayerAnimEventDo( centity_t *cent, animevent_t *animEvent )
{
	...
	case AEV_EFFECT:
		...
		if ( animEvent->eventData[AED_BOLTINDEX] != -1 )
		{
			...
			trap->FX_PlayEffectID(animEvent->eventData[AED_EFFECTINDEX], bPoint, bAngle, -1, -1, false);
		}
		else
		{
			vec3_t bAngle;
			...
			VectorSet(bAngle, 0, 1, 0);
			trap->FX_PlayEffectID(animEvent->eventData[AED_EFFECTINDEX], cent->lerpOrigin, bAngle, -1, -1, false);
		}
#endif
		break;
...
static void CG_SetLerpFrameAnimation( centity_t *cent, clientInfo_t *ci, lerpFrame_t *lf, int newAnimation, float animSpeedMult, qboolean torsoOnly, qboolean flipState) {
	...
	if (cent->localAnimIndex > 0 && cent->localAnimIndex != 2)
	...
	if (cent->ghoul2)
	{
		...
		{
			int weaponID = BG_GetWeaponIndexFromClass(cent->currentState.weapon, cent->currentState.weaponVariation);
			JKG_ReloadAnimation(cent->currentState.firingMode, weaponID, 
				cent->currentState.torsoAnim, bgAllAnims[cent->localAnimIndex].anims, &animSpeed);
		}
		...
		{ //rww - The guesswork based on the lerp frame figures is usually BS, so I've resorted to a call to get the frame of the bone directly.
			...
			for (i = 0; i < MAX_ARMOR; i++) {
				if (cent->currentState.armor[i])
					trap->G2API_SetBoneAnim(cent->ghoul2, 4 + i, "lower_lumbar", firstFrame, lastFrame, flags, animSpeed, cg.time, beginFrame, blendTime);
			}
		...
		else
		{
			...
			for (i = 0; i < MAX_ARMOR; i++) { 
				if (cent->currentState.armor[i])
					trap->G2API_SetBoneAnim(cent->ghoul2, 4 + i, "model_root", firstFrame, lastFrame, flags, animSpeed, cg.time, beginFrame, blendTime);
			}
		...
		if (cent->localAnimIndex < NUM_RESERVED_ANIMSETS && (cent->currentState.torsoAnim) == newAnimation && !cent->noLumbar)
		{ //make sure we're humanoid before we access the motion bone
			...
			for (i = 0; i < MAX_ARMOR; i++) {
				if (cent->currentState.armor[i])
					trap->G2API_SetBoneAnim(cent->ghoul2, 4 + i, "Motion", firstFrame, lastFrame, flags, animSpeed, cg.time, beginFrame, blendTime);
			}
		}
	}
}
...
static qboolean CG_FirstAnimFrame(lerpFrame_t *lf, qboolean torsoOnly, float speedScale)
{
	if (torsoOnly && lf->animationTorsoSpeed == speedScale)
	{
		return qfalse;
	}
...
static void CG_RunLerpFrame( centity_t *cent, clientInfo_t *ci, lerpFrame_t *lf, qboolean flipState, int newAnimation, float speedScale, qboolean torsoOnly) 
{
	...
	if (cent->currentState.forceFrame)
	{
		if (lf->lastForcedFrame != cent->currentState.forceFrame)
		{
			...
			for (i = 0; i < MAX_ARMOR; i++) {
				trap->G2API_SetBoneAnim(cent->ghoul2, 4 + i, "lower_lumbar", cent->currentState.forceFrame, cent->currentState.forceFrame + 1, flags, animSpeed, cg.time, -1, 150);
				trap->G2API_SetBoneAnim(cent->ghoul2, 4 + i, "model_root", cent->currentState.forceFrame, cent->currentState.forceFrame + 1, flags, animSpeed, cg.time, -1, 150);
				trap->G2API_SetBoneAnim(cent->ghoul2, 4 + i, "Motion", cent->currentState.forceFrame, cent->currentState.forceFrame + 1, flags, animSpeed, cg.time, -1, 150);
			}
		}
	...
	else if (JKG_HasFreezingBuff(&cent->currentState) && cent->currentState.freezeTorsoAnim && cent->currentState.freezeTorsoAnim < MAX_ANIM_FILES)
	{
	    ...
	    if ( !torsoOnly )
	    {
			...
			for (i = 0; i < MAX_ARMOR; i++) {
				trap->G2API_SetBoneAnim(cent->ghoul2, 4 + i, "model_root", legsAnimFrame, legsAnimFrame, BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, 1.0f, cg.time, -1, 150);
			}
	    }
		...
		for (i = 0; i < MAX_ARMOR; i++) {
			trap->G2API_SetBoneAnim(cent->ghoul2, 4 + i, "lower_lumbar", torsoAnimFrame, torsoAnimFrame, BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, 1.0f, cg.time, -1, 150);
		}
	}
	else
	{
	...
	{ //We're in a ragdoll state, so make the call to keep our positions updated and whatnot.
		...
		tParms.endFrame = bgAllAnims[cent->localAnimIndex].anims[ragAnim].firstFrame + bgAllAnims[cent->localAnimIndex].anims[ragAnim].numFrames;
		{
			...
			{ //lock the anim on the current frame.
				...
				for (i = 0; i < MAX_ARMOR; i++) {
					trap->G2API_SetBoneAnim(cent->ghoul2, 4+i, "lower_lumbar", currentFrame, currentFrame + 1, flags, animSpeed, cg.time, currentFrame, blendTime);
					trap->G2API_SetBoneAnim(cent->ghoul2, 4+i, "model_root", currentFrame, currentFrame + 1, flags, animSpeed, cg.time, currentFrame, blendTime);
					trap->G2API_SetBoneAnim(cent->ghoul2, 4+i, "Motion", currentFrame, currentFrame + 1, flags, animSpeed, cg.time, currentFrame, blendTime);
				}
			}
		}
		...
		for (int i = 0; i < MAX_ARMOR; i++) {
			if (cent->currentState.armor[i]) {
				CG_G2SetBoneAngles(cent->ghoul2, 4+i, "upper_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time);
				CG_G2SetBoneAngles(cent->ghoul2, 4+i, "lower_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time);
				CG_G2SetBoneAngles(cent->ghoul2, 4+i, "thoracic", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time);
				CG_G2SetBoneAngles(cent->ghoul2, 4+i, "cervical", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time);
			}
		}
		...
		{ //if it's non-0 then it could have something in it.
			...
			{ //got the bone, now set the angles from the corresponding entitystate boneangles value.
				...
				for (int j = 0; j < MAX_ARMOR; j++) {
					if (cent->currentState.armor[j]) {
						trap->G2API_SetBoneAngles(cent->ghoul2, 4 + j, boneName, boneAngles, flags, up, right, forward, cgs.gameModels, 100, cg.time);
					}
				}
			}
		}
...
//	float		timeScaleMod = (cg_timescale.value&&gent&&gent->s.clientNum==0&&!player_locked&&!MatrixMode&&gent->client->ps.forcePowersActive&(1<<FP_SPEED))?(1.0/cg_timescale.value):1.0;
	...
	for (i = 0; i < MAX_ARMOR; i++) {
		trap->G2API_SetBoneAnim(cent->ghoul2, 4 + i, "face", firstFrame, lastFrame, animFlags, animSpeed, cg.time, -1, blendTime);
	}
}
...
static void CG_G2PlayerAngles( centity_t *cent, vec3_t legs[3], vec3_t legsAngles)
{
	...
	{ //don't do these things on non-humanoids
		...
		VectorCopy(lookAngles, lookAnglesBefore);
...
//	int		team;
	...
	{
		// Exception: Dont draw this on ourselves if we're in a cinematic
		if (cent->currentState.number == cg.snap->ps.clientNum && cg.cinematicState) {
			// Dont draw
		} else {
	...
	while (!doneWithTraces)
	{
		...
		{ //hit a client..
			...
			{ //it succeeded with the ghoul2 trace
				trap->FX_PlayEffectID( cgs.effects.mSaberBloodSparks, trace.endpos, trace.plane.normal, -1, -1, false );
...
void CG_CreateNPCClient(clientInfo_t **ci)
{
	...
	*ci = (clientInfo_t *) malloc(sizeof(clientInfo_t));
}
...
void CG_DrawPlayerBBox ( const centity_t *cent )
{
    ...
    const clientInfo_t *ci = &cgs.clientinfo[s->number];

    vec3_t mins = { -15.0f, -15.0f, -24.0f };
    vec3_t maxs = { 15.0f, 15.0f, 40.0f };
    ...
    for ( i = 0; i < 4; i++ )
    {
        ...
        corners[i + 4][0] = maxs[0];
    }
    
    if ( s->number < MAX_CLIENTS )
    {
        switch ( ci->team )
        {
            case TEAM_RED:
    ...
    {
        // non-clients can have green bounding boxes!
        boxcolor[0] = 0;
        boxcolor[1] = 255;
        boxcolor[2] = 0;
        boxcolor[3] = 127;
    ...
    for ( i = 0; i < 24; i += 4 )
    {
        for ( j = 0; j < 4; j++ )
        {
            VectorCopy (corners[indices[i + j]], verts[i + j].xyz);
            memcpy (verts[i + j].modulate, boxcolor, sizeof (verts[i + j].modulate));
        }
    ...
    trap->R_AddPolysToScene (cgs.media.bboxShader, 4, &verts[0], 6);
}
...
void CG_Player( centity_t *cent ) {
	...
	if (cent->ghoul2 && 
		...
		cent->ghoul2weapon != CG_G2WeaponInstance(cent, cent->currentState.weapon, cent->currentState.weaponVariation) &&
		...
		cg.snap && (cent->currentState.number != cg.snap->ps.clientNum || (cg.snap->ps.pm_flags & PMF_FOLLOW)))
	{
		...
		{
			CG_CopyG2WeaponInstance(cent, cent->currentState.weapon, cent->currentState.weaponVariation, cent->ghoul2);
	...
	// JKG - Custom teams
	switch (ci->customteam)
	{
		default:	// No team
			break;
		case 1:		// Generic Red team (only render for teammates!)
			if (cgs.clientinfo[cg.predictedPlayerState.clientNum].customteam != 2) {
				CG_PlayerFloatSprite( cent, cgs.media.teamRedShader);
			}
			break;
		case 2:		// Generic Blue team (only render for teammates!)
			if (cgs.clientinfo[cg.predictedPlayerState.clientNum].customteam != 1) {
				CG_PlayerFloatSprite( cent, cgs.media.teamBlueShader);
			}
			break;
	}
...
//[TrueView]
	...
	if (cg.snap && cent->currentState.number == cg.snap->ps.clientNum)
	{
		if ( !cg.renderingThirdPerson && (cg_trueguns.integer || cent->currentState.weapon == WP_SABER 
			|| cent->currentState.weapon == WP_MELEE) && !cg.predictedPlayerState.zoomMode && !cg.cinematicState)	
		{
			//<True View varibles
...
SkipTrueView:
		...
		cent->currentState.number))
	{
		if (cent->ghoul2)
		{
			...
			trap->FX_PlayEntityEffectID(cgs.effects.mForceConfusionOld, efOrg, axis, -1, -1, -1, -1);
		}
	}
...
stillDoSaber:
	...
	if (drawPlayerSaber)
	{
		...
		else if (!cent->currentState.saberInFlight)
		{
			...
			{
				trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_SABER, 0), 0, cent->ghoul2, 1);
	...
	if (cent->currentState.weapon != WP_EMPLACED_GUN)
	{
		...
		if ( cg.predictedPlayerState.clientNum != cent->currentState.number ||
		    !JKG_ShouldRenderWeaponViewModel (cent, &cg.predictedPlayerState) )
		{
		    JKG_RenderWeaponWorldModel (cent, rootAngles);
		}
	}
	...
	JKG_DrawWeaponHolsters(cent, legs, shadowPlane);
