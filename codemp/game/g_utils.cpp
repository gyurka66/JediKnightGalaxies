...
int G_ModelIndex( const char *name ) {
	...
	trap->FS_FOpenFile(name, &fh, FS_READ);
	if (!fh)
	{ //try models/ then, this is assumed for registering models
		trap->FS_FOpenFile(va("models/%s", name), &fh, FS_READ);
		if (!fh)
		{
...
qboolean G_ClearTrace( vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int ignore, int clipmask )
{
	...
	trap->Trace( &tr, start, mins, maxs, end, ignore, clipmask , 0, 0, 0);
...
qboolean G_CheckInSolid (gentity_t *self, qboolean fix)
{
	...
	trap->Trace(&trace, self->r.currentOrigin, mins, self->r.maxs, end, self->s.number, self->clipmask, 0, 0, 0);
...
qboolean G_ExpandPointToBBox( vec3_t point, const vec3_t mins, const vec3_t maxs, int ignore, int clipmask )
{
	...
	for ( i = 0; i < 3; i++ )
	{
		...
		trap->Trace( &tr, start, vec3_origin, vec3_origin, end, ignore, clipmask , 0, 0, 0);
	...
	trap->Trace( &tr, start, mins, maxs, start, ignore, clipmask , 0, 0, 0);
	if ( tr.allsolid || tr.startsolid )
	{
		return qfalse;
	}
